...
from .models import Materiel, Tierce,Emprunt
...
class EmpruntForm(forms.Form):
    ...
    def clean(self):
        ...
        return cleaned
class EmpruntEditForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Exemple : champ ID non modifiable (si ton modèle en a un)
        if "id_Emprunt" in self.fields:
            self.fields["id_Emprunt"].widget.attrs["readonly"] = True
            self.fields["id_Emprunt"].widget.attrs["class"] = "field bg-slate-100"
            self.fields["id_Emprunt"].help_text = "ID non modifiable."
