...
from .form import EditEmprunteurForm, EditMaterielForm, EnregistrerEmprunteurForm, MaterielForm, EmpruntForm,EmpruntEditForm
...
@login_required
def modifier_emprunt_view(request: HttpRequest, emprunt_id: str) -> HttpResponse:
    emprunt = get_object_or_404(Emprunt, pk=emprunt_id)

    if request.method == "POST":
        form = EmpruntEditForm(request.POST, instance=emprunt)
        if form.is_valid():
            form.save()
            messages.success(
                request,
                f"Emprunt modifié avec succès."
            )
            return redirect( (f"{reverse('equipement:dashboard')}?tab=lister_emprunts")            )
        messages.error(request, "Veuillez corriger les erreurs du formulaire.")

    else:
        form = EmpruntEditForm(instance=emprunt)

    return render(request, "equipement/edit_emprunt.html", {
        "form": form,
        "emprunt": emprunt,
    })