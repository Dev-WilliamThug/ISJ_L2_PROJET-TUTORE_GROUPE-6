...
def dashboard(request: HttpRequest) -> HttpResponse:
    # ✅ OPTION B — email auto au premier retard détecté
    if request.user.is_authenticated:
...
def exporter_excel(request):
    ...
    ws = wb.active
    ws.title = "Materiels"
    ws.append(["id_materiel", "nom", "couleur", "categorie", "etat", "marque"])
    for m in Materiel.objects.all():
        ws.append([str(m.id_materiel), m.nom or "", m.couleur or "",
                   m.categorie or "", m.etat or "", m.marque or ""])
...
def import_materiels(request):
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        ...
        Materiel.objects.update_or_create(
            ...
            defaults={"nom": nom, "couleur": couleur, "categorie": categorie,
                      "etat": etat, "marque": marque},
        )
        importes += 1
...
def import_emprunts(request):
    ...
    for i, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
        ...
        emprunt_id, noms_materiels, nom_emprunteur = row[0], row[1], row[2]
        retour_prevu, notes = row[4], row[5]
        ...
        if len(parties) >= 2:
            emprunteur_obj = Tierce.objects.filter(
                prenom__iexact=parties[0], nom__iexact=" ".join(parties[1:])
            ).first()
            if not emprunteur_obj:
                emprunteur_obj = Tierce.objects.filter(
                    nom__iexact=parties[0], prenom__iexact=" ".join(parties[1:])
                ).first()
        if not emprunteur_obj:
        ...
        for nom in noms_liste:
            try:
                materiels_objs.append(Materiel.objects.get(nom__iexact=nom))
                continue
            except Materiel.DoesNotExist:
                pass
            except Materiel.MultipleObjectsReturned:
                erreurs.append(f"Ligne {i} : plusieurs matériels pour '{nom}'")
                ligne_ok = False
                break
            try:
                materiels_objs.append(Materiel.objects.get(id_materiel__iexact=nom))
            except Materiel.DoesNotExist:
                erreurs.append(f"Ligne {i} : matériel '{nom}' introuvable")
                ligne_ok = False
                break
        ...
        try:
            ...
            if emprunt_id_int:
                emprunt, created = Emprunt.objects.update_or_create(
                    id=emprunt_id_int,
                    defaults={"emprunteur": emprunteur_obj,
...
def exporter_template_emprunts(request):
    ...
    for emprunt in Emprunt.objects.prefetch_related("lignes__materiel").select_related("emprunteur").order_by("id"):
        lignes = emprunt.lignes.all()
        noms_materiels = ", ".join(l.materiel.nom for l in lignes) if lignes.exists() else ""
        ws.append([
            emprunt.id,
            noms_materiels,
            f"{emprunt.emprunteur.prenom} {emprunt.emprunteur.nom}",
            emprunt.created_at.strftime("%Y-%m-%d %H:%M:%S") if emprunt.created_at else "",
...
def overdue_emprunts(request: HttpRequest) -> HttpResponse:
    ...
    ✅ OPTION B : déclenche aussi l'email auto pour les nouveaux retards.
    """
    # Email automatique au premier retard détecté
    ReminderService.check_and_notify_new_overdue()
...
def marquer_rendu(request: HttpRequest, emprunt_id: int) -> HttpResponse:
    ...
    Marque un emprunt comme rendu :
      - statut → RETOURNE
      - date_retour_reelle = aujourd'hui
      - remet chaque matériel en DISPONIBLE
      - envoie un email de confirmation à l'emprunteur
    """
...
# ------------------------------------------------------------------ #
#  API notifications (cloche header)                                  #
# ------------------------------------------------------------------ #
...
def send_reminders_manual(request: HttpRequest) -> HttpResponse:
    """Bouton manuel : envoie les rappels périodiques (J+1, J+3, J+7)."""
    try:
